...
class db_split {
	...
	private $_data = null;
	private $_options = array('in:',						/* Input SQL dump. */
														'out:',						/* Output directory to store results. */
														'list::',					/* List all tables from SQL dump. Ignores all other options. */
														'force::',				/* Force create output directory it does not exist. */
														'postfix-time:',	/* Postfix for table export - MUST be in double quotes and in valid date() format -> http://php.net/manual/en/function.date.php */
														'postfix-name:',	/* Postfix name for table export */
														'ignore:',				/* Which tables to ignore. Comma separated */
														'only:',					/* Specific tables to export. This will override the --ignore option (if set) */
														);
			...
			if (substr($line, 0, $length) == $this->_TABLE_BEGIN) {
				...
				if ($this->_data->list) {
					...
					if (count($this->_data->specific) > 0) {
						if (!in_array($table_name, $this->_data->specific)) { continue; }
					} else if (count($this->_data->ignore) > 0) {
						if (in_array($table_name, $this->_data->ignore)) { continue; }
					}
					
					// Compose the table's name.
					$table = $table_name;
					if (!empty($this->_data->postfix_name)) { $table .= '-' . $this->_data->postfix_name; }
					if (!empty($this->_data->postfix_time)) { $table .= '-' . $this->_data->postfix_time; }
	...
	private function parse_arguments() {
		...
		if (isset($o['out']) && !empty($o['out'])) {
			if (!is_dir($o['out'])) {
				if (!$this->_data->force) {
					$this->abort('Fatal error: Output directory does not exist and the --force option was not used.', true);
				}
